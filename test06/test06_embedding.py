...
def load_and_sample_stgcn_tensor(frame_paths, keypoint_dir, start, end, target_patient_id, num_frames=16, conf_threshold=0.05):
    """
    관절 좌표 보간 -> 💡양어깨 기준 영점 조준 -> ST-GCN 규격 텐서 변환
    """
        ...
        if json_path.exists():
            with open(json_path, 'r') as f:
                ...
                if instance and len(instance.get('keypoints', [])) >= 17:
                    for v, kp in enumerate(instance['keypoints'][5:17]): # 12개 관절
...
if integrated_save_path.exists():
    ...
    for target in tqdm(range(len(df)), desc="전체 진행률"):
        ...
        for clip in get_all_clips(Path(paths['ass']), len(frame_paths)):
            # 💡 [수정] current_patient_id 를 파라미터로 넘겨줌
            stgcn_tensor = load_and_sample_stgcn_tensor(
                frame_paths, KEYPOINT_DIR, clip['start'], clip['end'], current_patient_id, num_frames=16
            ).to(device)
            
            with torch.no_grad():
                embedding = model(stgcn_tensor)
